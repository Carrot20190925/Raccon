...
class TXTheme: NSObject {
    ...
    static func sixthColor() -> UIColor{
        return self.rgbColor(60, 60, 60)
    }
    static func fifthColor() -> UIColor{
        return self.rgbColor(109, 109, 109)
    }
    static func whiteColor() -> UIColor{
        return UIColor.white
    }
    static func titleColor() -> UIColor{
        return self.rgbColor(42, 42, 42)
    }
    static func secondColor() -> UIColor{
        return self.rgbColor(26, 26, 26)
    }
...
//        }
    ...
    static func formartString(string : String) -> String{
           ...
           let num = string.count / 3
           var remainder = string.count % 3
           ...
           if num > 0,remainder > 0 {
               ...
               thirdString = string.substring(from: string.count - (num + 1))
               parserText = string.substring(to: string.count - thirdString.count)
               remainder = parserText.count % num
               if remainder > 0 {
                   secondString = parserText.substring(from: parserText.count - (num + 1))
               }else{
                   secondString = parserText.substring(from: parserText.count - num)
               }
               firstString = parserText.substring(to: parserText.count - secondString.count)
               
               return firstString + " " + secondString + " " + thirdString
               
           }else if num > 0, remainder == 0{
               thirdString = string.substring(from: string.count - (num))
               parserText = string.substring(to: string.count - thirdString.count)
               secondString = parserText.substring(from: parserText.count - num)
               firstString = parserText.substring(to: parserText.count - secondString.count)
               return firstString + " " + secondString + " " + thirdString

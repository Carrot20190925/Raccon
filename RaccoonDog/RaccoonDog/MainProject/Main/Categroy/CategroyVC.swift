...
class CategroyVC: BaseController {
    ...
    var titleView = UIView.init(frame: CGRect.init(x: 0, y: 0, width: rScreenWidth, height: 44))
    var titleScrollView = UIScrollView.init(frame: CGRect.init(x: 0, y:0 , width: rScreenWidth, height: 44))
//    var searchButton = UIButton.init(frame: CGRect.init(x: rScreenWidth - 60, y: StatusBarHeight, width: 60, height: 0))
    ...
    var bodyView = UIScrollView.init(frame: CGRect.init(x: 0, y: 44, width: rScreenWidth, height: rScreenHeight - NavgationBarHeight - 44))
...
extension CategroyVC:UIScrollViewDelegate{
...
//        self.searchButton.setTitleColor(TXTheme.titleColor(), for: .normal)
        ...
        for (index,title) in titles.enumerated() {
            ...
            let listVC = CategoryListVC.init(collectionViewLayout: UICollectionViewFlowLayout.init())
...
//        var offsetX : CGFloat = 0
        ...
        if x > halfW,(x + halfW) < titleContentViewW  {
            let  offsetX = x - halfW + sender.mj_w * 0.5

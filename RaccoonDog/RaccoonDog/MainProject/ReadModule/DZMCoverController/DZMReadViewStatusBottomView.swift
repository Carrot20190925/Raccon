...
/// bottomView 高度
//let DZM_READ_STATUS_BOTTOM_VIEW_HEIGHT:CGFloat =  DZM_SPACE_30
...
class DZMReadViewStatusBottomView: UIView {
    ...
    private func addSubviews() {
        ...
        batteryView.tintColor = ReadConfigModel.shared().statusColor
        addSubview(batteryView)
        
        // 时间
        timeLabel = UILabel()
        timeLabel.textAlignment = .center
        timeLabel.font = TXTheme.thirdTitleFont(size: 11)
        timeLabel.textColor = ReadConfigModel.shared().statusColor
        addSubview(timeLabel)
        
        // 进度
        progress = UILabel()
        progress.font = TXTheme.thirdTitleFont(size: 11)
        progress.textColor = ReadConfigModel.shared().statusColor
        addSubview(progress)
